...
export default function LightMode() {
    ...
    const readStoredMode = (): 0 | 1 | 2 => {
        if (typeof window === 'undefined') return 2;
        const raw = window.localStorage.getItem(STORAGE_KEY);
        if (raw === null) return 2;
        const num = Number(raw);
        return num === 0 || num === 1 || num === 2 ? (num as 0 | 1 | 2) : 2;
    };
    ...
    const isDark = useMemo(() => {
        return mode === 1 || (mode === 2 && systemDark);
    }, [mode, systemDark]);
    ...
    useEffect(() => {
        ...
        media.addEventListener
            ...
            : media.addListener(onChange as any);
        setSystemDark(media.matches);
        return () => {
            media.removeEventListener
                ? media.removeEventListener('change', onChange)
                : media.removeListener(onChange as any);
        };
    }, []);
    ...
    const aria = 'Toggle theme';

    return (
        <button
            className="hidden md:block fixed bottom-0 right-0 m-2 bg-white/10 hover:bg-white/20 dark:bg-black/10 dark:hover:bg-black/20 w-10 h-10 p-2 rounded-[8px] cursor-pointer"
            onClick={handleClick}
            aria-label={aria}
        >
            <svg
